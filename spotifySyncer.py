...
# pip install argparse spotipy pandas openpyxl
...
class Syncer:
    ...
    def via_argparse(cls):
        ...
        parser.add_argument(
            "-id",
            default=environ.get("SPOTIFY_CLIENT_ID"),
            help="spotify client id (can be specified with ENV:SPOTIFY_CLIENT_ID)",
        )
        parser.add_argument(
            "-secret",
            default=environ.get("SPOTIFY_CLIENT_SECRET"),
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="proxies in http format like http://127.0.0.1:8080",
        )
    ...
    def sync(self):
        ...
        for i in range(3):
            try:
                ...
                send_notification(
                    f"spotifySyncer Error {i + 1}", f"{e.__class__.__name__}{e.args}"
                )
                sleep(30)
    ...
    def read(self, source):
                ...
                try:
                    tracks = (
                        self.sp.current_user_saved_tracks(
                            limit=50, offset=len(all_tracks)
                        )
                        if source == "saved_tracks"
                        else self.sp.playlist_items(
                            source, limit=100, offset=len(all_tracks)
                        )
                    )
                    break
                except Exception:
        ...
        all_tracks["added_at"] = DatetimeIndex(
            to_datetime(all_tracks["added_at"], utc=True)
        ).tz_convert(None)

        return all_tracks

    def update(self, tracks_orgn, tracks_dst):
        drop_duplicated = concat([tracks_orgn, tracks_dst]).drop_duplicates(
            ["id"], keep=False
        )
        ...
        if not new_saved_tracks.empty:
            new_saved_tracks = new_saved_tracks.sort_values(
